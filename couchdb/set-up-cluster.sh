...
CLUSTER_PEERS=(${CLUSTER_PEER_IPS//,/ })
NODE_COUNT=$((${#CLUSTER_PEERS[@]}+1))

verify_membership(){
    curl -s http://$COUCHDB_USER:$COUCHDB_PASSWORD@$SVC_NAME:5984/_membership | jq "(.all_nodes | length) == $NODE_COUNT"
}
...
enable_cluster_on_remote_node(){
    peer=$1
    curl -X POST -H "Content-Type: application/json" http://$COUCHDB_USER:$COUCHDB_PASSWORD@$SVC_NAME:5984/_cluster_setup \
    -d '{"action": "enable_cluster", "bind_address":"0.0.0.0", "username": "'$COUCHDB_USER'", "password":"'$COUCHDB_PASSWORD'", "port": 5984, "node_count": "'$NODE_COUNT'", "remote_node": "'$peer'", "remote_current_user": "'$COUCHDB_USER'", "remote_current_password": "'$COUCHDB_PASSWORD'" }'
}

join_node_to_cluster(){
    peer=$1
    curl -X POST -H "Content-Type: application/json" http://$COUCHDB_USER:$COUCHDB_PASSWORD@$SVC_NAME:5984/_cluster_setup \
    -d '{"action": "add_node", "host":"'$peer'", "port":5984, "username": "'$COUCHDB_USER'", "password":"'$COUCHDB_PASSWORD'"}'
}
...
add_peer_to_cluster(){
    ...
    echo "Node $peer is already in the cluster"
      ...
      enable_cluster_on_remote_node $peer
      join_node_to_cluster $peer
    fi
}
